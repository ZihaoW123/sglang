...
class MambaPoolHost(HostKVCache):
    def __init__(
        ...
        logger.info(
            "[MambaPoolHost][INIT] host_size=%d device_size=%d "
            "num_layers=%d layout=%s temporal_shape=%s conv_shapes=%s",
            self.size,
            device_pool.size,
            self.num_mamba_layers,
            self.layout,
            tuple(device_pool.mamba_cache.temporal.shape),
            [tuple(c.shape) for c in device_pool.mamba_cache.conv],
        )
